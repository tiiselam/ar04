...
ALTER view [dbo].[vwTII_IMPFACELECT_TEST_PRD01] as
		SELECT 
		'SOP30200' stabla,
...
--		,	SP102.SERLTQTY												QUANTITY
		...
		,	CASE WHEN SP302.ORIGNUMB <> '' AND SP302.SOPTYPE = 3 THEN ISNULL((SELECT TOP 1 ORG.ORIGNUMB FROM
			(SELECT ORIGNUMB FROM SOP30200 X WHERE X.SOPTYPE = SP302.ORIGTYPE AND X.SOPNUMBE = SP302.ORIGNUMB
			UNION
			SELECT ORIGNUMB FROM SOP10100 X WHERE X.SOPTYPE = SP302.ORIGTYPE AND X.SOPNUMBE = SP302.ORIGNUMB) ORG), '') 
			else
...
--					)													IMPVAL02
		...
		,	CASE WHEN FEI_COD in(201,206,211) then  
					rtrim(FORMAT(ISNULL(FEI.FEI_VtoPago	   ,'01-01-1900'),'dd-MM-yyyy'))
				 ELSE
					CONVERT(VARCHAR,SP302.DUEDATE,103)	
				END 							DUEDATE
...
--							AND SP105.TAXDTLID IN ('V-IVA 21 DESG','V-IVA 10.5 DESG') 
		...
		,	ISNULL((SELECT TOP 1 SERLTNUM
			FROM SOP10201 SP102 WHERE SP102.SOPTYPE						=	SP303.SOPTYPE
		...
		,	(SELECT Substring(INFO.INETINFO,charindex('CBU=',INFO.INETINFO,1)+4,22) CBU
			FROM DYNAMICS.dbo.SY01500    AS CMP 
			LEFT OUTER JOIN SY01200 AS INFO ON CMP.INTERID = INFO.MASTER_ID 
			WHERE CMP.INTERID = DB_NAME()
			 AND  INFO.ADRSCODE = 'PRINCIPAL' 
			 AND  INFO.MASTER_TYPE = 'CMP') AS CBU		
		,	CASE WHEN RTRIM(RM101.COMMENT1) = 'ARS' THEN 
				ISNULL	( 
						( 
						SELECT	SUM ( SP105.STAXAMNT ) 
...
----------------------------------
		...
		FROM SOP30200							SP302
		     INNER JOIN	SOP30300	SP303	ON	SP303.SOPTYPE						=	SP302.SOPTYPE
...
--		AND	IV301.LOTNUMBR						=	SP102.SERLTNUM
	    ...
	    LEFT OUTER JOIN
			FEI_SOP30200 AS FEI	ON SP302.SOPTYPE	= FEI.SOPTYPE AND SP302.SOPNUMBE  = FEI.SOPNUMBE
		WHERE
			SP302.SOPTYPE						IN	( 3 , 4 )
		--AND	SP302.SOPNUMBE						BETWEEN	@SOPNUMBED AND @SOPNUMBEH
		AND	SP302.VOIDSTTS						=	0
		AND SP303.QUANTITY						<>	0
		--AND SP302.SOPNUMBE in( 'FV B0004-00000003','FV A0004-00005566')
